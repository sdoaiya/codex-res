...
async function resolveCandidate(candidateId, options) {
...
async function collectCodexJsonlFiles(rootPath) {
    const results = [];
...
function findThreadIdsNeedingDbSync(rows, provider, model) {
    return rows.filter((row) => row.modelProvider !== provider || (!!model && row.model !== model) || row.archived !== 0);
}
async function collectCodexState(candidate) {
    ...
    const rows = db
        .prepare("SELECT id, model_provider AS modelProvider, model, archived, COALESCE(rollout_path, '') AS rolloutPath FROM threads")
        .all();
    db.close();
    return {
        codexHome,
...
function findMismatchedJsonlFiles(files, provider, model) {
    const mismatched = [];
...
function readCursorThreadStates(dbPath, transcriptIds) {
    const db = new DatabaseSync(dbPath, { open: true, readOnly: true });
...
export async function diagnoseEnvironment(candidateId, options) {
...
export async function previewRepair(candidateId, options) {
    const candidate = await resolveCandidate(candidateId, options);
    if (candidate.product === "cursor") {
        const transcriptFiles = await collectCodexJsonlFiles(candidate.artifacts.transcriptsRootPath ?? "");
        const transcriptIds = transcriptFiles.map((filePath) => path.basename(filePath, ".jsonl"));
...
function patchJsonlFile(filePath, provider, model) {
    const content = fsSync.readFileSync(filePath, "utf8");
...
export async function applyRepair(planId, _options) {
    ...
    if (record.plan.product === "cursor") {
        ...
        for (const transcriptId of cursorRecord.transcriptIds) {
            insert.run(`composerData:${transcriptId}`, JSON.stringify({
                composerId: transcriptId,
                fullConversationHeadersOnly: [],
                status: "restored"
            }));
            insert.run(`checkpointId:${transcriptId}:${randomUUID()}`, JSON.stringify({
                files: [],
                nonExistentFiles: [],
                newlyCreatedFolders: [],
                activeInlineDiffs: [],
...
export async function restoreLatestBackup(candidateId, options) {
    const candidate = await resolveCandidate(candidateId, options);
    const backupRoot = candidate.product === "codex"
